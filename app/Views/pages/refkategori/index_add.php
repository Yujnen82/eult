                ...
                <div class="kt-portlet__head">
                    <div class="kt-portlet__head-label">
                        <h3 class="kt-portlet__head-title">
                            <?= strtoupper(esc($page_judul ?? 'Sub Layanan')) ?>
                        </h3>
                    </div>
                    <div class="kt-portlet__head-toolbar">
                        <div class="kt-portlet__head-actions">
                            <a href="<?= ($create_url ?? '#') . ($catId ?? '') ?>" class="btn btn-brand btn-elevate btn-icon-sm" id="btn-add">
                                ...
                                <table class="table table-hover table-striped mb-0">
                                    <thead class="thead-light">
                                        <tr>
                                            ...
                                            <th class="text-uppercase text-muted font-weight-bold" style="font-size: 11px; letter-spacing: 0.5px;">Nama Sub Layanan</th>
                                    ...
                                    <tbody>
                                        <?php if (!empty($datas)): ?>
                                            ...
                                            foreach ($datas as $row):
                                                $key = service('enkripsi')->encode($row['sCatId']);
                                            ?>
                                                <tr>
                                                    <td class="text-center font-weight-bold"><?= $i++ ?></td>
                                                    <td class="font-weight-bold text-dark"><?= esc($row['sCatNama']) ?></td>
                                                    <td class="text-center">
                                                        <a href="<?= ($update_url ?? '#') . $key ?>" title="Ubah Data" class="btn btn-sm btn-label-brand btn-bold" id="btn-edit">
